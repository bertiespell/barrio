...
contract("Listings.confirmBuy", function (accounts) {
  ...
  it("should refund other buyers held in escrow", async () => {
    ...
    const contractBalanceAfterOffers = await web3.eth.getBalance(listingContract.address);
    assert.equal(BigInt(contractBalanceAfterOffers), BigInt(contractBalanceBefore) + BigInt(itemPrice * 3));
    ...
    const contractBalanceAfterConfirm = await web3.eth.getBalance(listingContract.address);
    assert.equal(BigInt(contractBalanceAfterConfirm), BigInt(contractBalanceBefore));
  });
...
contract("Listings.checkUpkeep false", function (accounts) {
  it("should return false for newly added listings", async () => {
    ...
    assert.equal(upkeep.performData, web3.eth.abi.encodeParameter('string[]', []));
  });
});
...
contract("Listings.checkUpkeep true", function (accounts) {
  it("should return true for listings over a week old", async () => {
    ...
    assert.equal(upkeep.performData, web3.eth.abi.encodeParameter('string[]', ["QmV9tSDx9UiPeWExXEeH6aoDvmihvx6jD5eLb4jbTaKGps"]));
  });
});
...
contract("Listings.checkUpkeep multiple true", function (accounts) {
  it("should return true for listings over a week old", async () => {
    ...
    assert.equal(upkeep.upkeepNeeded, true);
    assert.equal(upkeep.performData, web3.eth.abi.encodeParameter(
      'string[]', 
      [
        "QmV9tSDx9UiPeWExXEeH6aoDvmihvx6jD5eLb4jbTaKGps", 
        "QmRAQB6YaCyidP37UdDnjFY5vQuiBrcqdyoW1CuDgwxkD4", 
